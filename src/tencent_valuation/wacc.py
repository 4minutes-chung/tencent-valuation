...
def run_wacc(
    ...
    rf_annual = float(monthly_window["RF"].mean() * 12.0)
    rf_weekly = float(monthly_window["RF"].mean() / 4.345)
    erp_annual = float(monthly_window["MKT_EXCESS"].mean() * 12.0)
    ...
    re_capm = capm_cost_of_equity(rf_annual, beta_l_tencent, erp_annual)
    ...
    rd = calc_rd(
        interest_expense_hkd_bn=interest_expense,
        avg_gross_debt_hkd_bn=debt,
        floor=float(wacc_config["rd_floor"]),
        ceiling=float(wacc_config["rd_ceiling"]),
    )
    ...
    beta_window_primary = f"{apt_lookback}m"
    secondary_windows = sorted([estimate.window_months for estimate in window_estimates if estimate.window_months != apt_lookback])
        ...
        [
            {
                ...
                "lambda_mkt": primary_estimate.lambdas_guardrailed["MKT_EXCESS"],
                "lambda_smb": primary_estimate.lambdas_guardrailed["SMB"],
                "lambda_hml": primary_estimate.lambdas_guardrailed["HML"],
            }
        ]
    )
