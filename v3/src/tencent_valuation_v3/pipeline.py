...
def _run_multimethod(
...
) -> tuple[ApvArtifacts, ResidualIncomeArtifacts, CompsArtifacts, SotpArtifacts, ReverseDcfArtifacts]:
    apv_artifacts = run_apv(asof, ctx.paths, dcf_artifacts.valuation_outputs, wacc_artifacts.wacc_components)
    ...
    comps_artifacts = run_comps(asof, ctx.paths, ctx.peers, wacc_artifacts.wacc_components)
    ...
    return apv_artifacts, residual_artifacts, comps_artifacts, tvalue_artifacts, reverse_artifacts
...
def apv_step(
    ...
    ctx = load_context(project_root)
    _, wacc_artifacts, _, dcf_artifacts = _run_core(asof, ctx, refresh_factors, source_mode)
    apv_artifacts, _, _, _, _ = _run_multimethod(asof, ctx, wacc_artifacts, dcf_artifacts)
...
def residual_income_step(
    ...
    ctx = load_context(project_root)
    _, wacc_artifacts, _, dcf_artifacts = _run_core(asof, ctx, refresh_factors, source_mode)
    _, residual_artifacts, _, _, _ = _run_multimethod(asof, ctx, wacc_artifacts, dcf_artifacts)
...
def comps_step(
    ...
    ctx = load_context(project_root)
    _, wacc_artifacts, _, dcf_artifacts = _run_core(asof, ctx, refresh_factors, source_mode)
    _, _, comps_artifacts, _, _ = _run_multimethod(asof, ctx, wacc_artifacts, dcf_artifacts)
...
def reverse_dcf_step(
    ...
    ctx = load_context(project_root)
    _, wacc_artifacts, _, dcf_artifacts = _run_core(asof, ctx, refresh_factors, source_mode)
    _, _, _, _, reverse_artifacts = _run_multimethod(asof, ctx, wacc_artifacts, dcf_artifacts)
...
def qa_step(
    ...
    run_ensemble(
        ...
        ctx.paths,
        ctx.method_weights,
        ctx.paths.reports / f"qa_{asof}.json",
        wacc_artifacts.wacc_components,
    )
...
def run_all(
    ...
    apv_artifacts, residual_artifacts, comps_artifacts, tvalue_artifacts, reverse_artifacts = _run_multimethod(
        asof,
        ctx,
        wacc_artifacts,
        dcf_artifacts,
